...
class WNXMLParserContentHandler(xml.sax.ContentHandler):
        ...
        if name == "WNXML":
            ...
            if self.m_done == 0:
                raise WNXMLParserException(
                    "WNXMLParser internal error: SYNSET should start now, but m_done is not 0 ({0})!".format(
                        self.m_done))
            self.m_done = 0
    ...
    def characters(self, chrs):
        ...
        if parent == "ID" and gparent == "SYNSET":  # SYNSET/ID
            ...
            if len(self.m_syns.synonyms) == 0:
                raise WNXMLParserException(
                    "WNXMLParser internal error: synonyms empty({0}) at LNOTE tag".format(len(self.m_syns.synonyms)))
