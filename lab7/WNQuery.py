...
class WNQuery:
    ...
    def __init__(self, wnxmlfilename, log=sys.stderr):
        ...
        self.taxonomy_depth = {}   # PoS -> taxonomy depth
        for pos in 'nvjb':
            self.compute_taxonomy_depth(pos)
    ...
    def writeStats(self, os):
        ...
        print("Adjectives\t{0}\t\t{1}\t\t{2}".format(len(self.dat("j")), aidx_len, len(self.idx("j"))), file=os)
            ...
            for synset_id, rel in sorted(val.ilrs):
                ...
                if rel in self._invRelTable:
                    ...
                    if synset_id not in dat:
                        print("Warning W03: synset {0} is missing ('{1}' target from synset {2})".format(synset_id, rel,
                                                                                                         key),
                              file=self.log)
                    else:
                        tt = dat[synset_id]
                        ...
                        if tt.wnid == val.wnid:
                            print("Warning W04: self-referencing relation '{0}' for synset {1}".format(invr, val.wnid),
                                  file=self.log)
                        else:
                            # add inverse to target synset
                            tt.ilrs.append((key, invr))
                            print("Added inverted relation (target={0},type={1}) to synset {2}".format(key, invr,
                                                                                                       tt.wnid),
                                  file=self.log)
    ...
    def simLeaCho(self, wnid1, wnid2, pos, relation, addArtificialTop):
        ...
        ci_r1 = None
        ci_r2 = None
        path_length = 2 * self.taxonomy_depth[pos]
        ...
        if ci_r1 and ci_r2:  # based on length of shortest connecting path
            return -1.0 * math.log10(float(path_length) / (2.0 * self.taxonomy_depth[pos]))
    ...
    def compute_taxonomy_depth(self, pos):
        taxonomy_depth = 0
        for synset_id in self.dat(pos):
            taxonomy_depth = max(taxonomy_depth, self.getMaxDepth(synset_id, pos, 'hypernym'))
        self.taxonomy_depth[pos] = taxonomy_depth
